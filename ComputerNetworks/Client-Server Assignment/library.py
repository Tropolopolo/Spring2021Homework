...
def ConnectClientToServer(server_sock):
    ...
    #client = CreateClientSocket(addr, 7777)
    
    return conn, (addr, server_sock.getsockname()[1])
...
class KeyValueStore(object):
  ...
  def __init__(self):
      """
      Idea: Have two dictionaries, one to hold the value the other to hold the time cached.
      """
      self.dictionary = {"":""}
      self.times = {"":(0.0,"")}
  ...
  def GetValue(self, key, max_age_in_sec=None):
    ...
    if(len(self.dictionary) == 0):
        ...
        if(max_age_in_sec != None):
            if((time.time() - self.times.get(key)) > max_age_in_sec):
                return "None"
            else:
  ...
  def StoreValue(self, key, value):
    ...
    """
    x = {key:value}
    self.dictionary.update(x)
    
    
  def StoreCache(self, command_line, response, time):
      """Stores the command_line and the response of the command_line
      """
      y = {command_line:(time, response)}
      self.times.update(y)
      
  def CheckCache(self, command_line, maxTime):
      val = self.times.get(command_line)
      if(val == None):
          return 0 
      if(time.time() - val[0] < maxTime):
          print("Sending from Cache: " + val[1])
          return val[1]
      else:
          del self.times[command_line]
          return 0
  ...
  def delCheck(self, maxTime):
      keys = self.times.keys()
      for key in keys:
          x = self.times.get(key)
          if(time.time() - x[0] > maxTime):
              del self.times[key]
